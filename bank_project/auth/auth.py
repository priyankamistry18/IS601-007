...
def check_duplicate(e):
    if 'Duplicate' in str(e):
        import re
        r = re.findall(r"'(.*?)'", str(e))
        if len(r) > 0:
            if r[1] == 'username':
                flash(f"The chosen {r[1]} '{r[0]}' has already been taken", "danger")
            elif r[1] == 'email': 
                flash(f"The {r[1]} '{r[0]}' is already registered", "danger") 
            else:
                flash(f"The chosen {r[1]} '{r[0]}' has already been used", "danger") 
        else:
